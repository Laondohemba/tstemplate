        ...
        <main class="pt-16 p-4">
            <div class="container mx-auto bg-gray-50 py-4 max-w-4xl min-h-screen">
                ...
                <div class="mb-6 px-4">
                    ...
                    <p class="text-green-600 mt-2">Update the details of your product.</p>
                </div>
                ...
                <div class="bg-white rounded-xl shadow-sm p-6 md:p-8">
                    <form id="productForm" method="POST" action="{{ route('products.update', $product->slug) }}" enctype="multipart/form-data" x-data="formSubmit" @submit.prevent="submit">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <select name="sales_type"
                                        ...
                                        <option value="wholesale" {{ old('sales_type', $product->sales_type) == 'wholesale' ? 'selected' : '' }}>Wholesale</option>
                                        <option value="retail" {{ old('sales_type', $product->sales_type) == 'retail' ? 'selected' : '' }}>Retail</option>
                                        <option value="both retail and wholesale" {{ old('sales_type', $product->sales_type) == 'both retail and wholesale' ? 'selected' : '' }}>Both wholesale and retail</option>
                                    </select>
                                    @error('sales_type')
                                        <span class="error-message text-red-500 text-sm mt-1 block">{{ $message }}</span>
                                    @enderror
                                ...
                                <div>
                                    ...
                                    <div class="flex space-x-4">
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="sales_niche" value="organic" {{ old('sales_niche', $product->sales_niche) == 'organic' ? 'checked' : '' }}
                                                class="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300">
                                            <span class="ml-2 text-gray-700">Organic</span>
                                        </label>
                                        <label class="inline-flex items-center">
                                            <input type="radio" name="sales_niche" value="inorganic" {{ old('sales_niche', $product->sales_niche) == 'inorganic' ? 'checked' : '' }}
                                                class="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300">
                        ...
                        <div id="step2" class="step-content hidden">
                            <div class="space-y-6">
                                ...
                                <div>
                                    ...
                                    <select name="currency"
                                        ...
                                        @foreach ($currencies as $currency)
                                            <option value="{{ $currency['sign'] }}" {{ old('currency', $product->currency) === $currency['sign'] ? 'selected' : '' }}>
                                ...
                                <div>
                                    ...
                                    <input type="number" name="quantity_available" value="{{ old('quantity_available', $product->quantity_available) }}" min="0"
                                        class="w-full px-4 py-3 border border-green-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-300 focus:border-green-300 transition-colors"
                                ...
                                <div class="pt-4 border-t border-gray-200">
                                    ...
                                    <div class="space-y-3">
                                        <label class="flex items-center">
                                            <input type="radio" name="shipping" value="local" {{ old('shipping', $product->shipping) == 'local' ? 'checked' : '' }}
                                        ...
                                        <label class="flex items-center">
                                            <input type="radio" name="shipping" value="international" {{ old('shipping', $product->shipping) == 'international' ? 'checked' : '' }}
                                                class="h-4 w-4 text-green-600 focus:ring-green-200 border-gray-300 rounded">
                                            <span class="ml-2 text-gray-700">International Shipping</span>
                                        </label>
                                        <label class="flex items-center">
                                            <input type="radio" name="shipping" value="both local and international" {{ old('shipping', $product->shipping) == 'both local and international' ? 'checked' : '' }}
                                                class="h-4 w-4 text-green-600 focus:ring-green-200 border-gray-300 rounded">
                                ...
                                <!-- Product Status -->
                                <div class="pt-4 border-t border-gray-200">
                                    <label class="block text-sm font-medium text-gray-700 mb-2">Product Status <span class="text-red-500">*</span></label>
                                    <select name="status"
                                        class="w-full px-4 py-3 border border-green-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-300 focus:border-green-300 transition-colors">
                                        ...
                                        <option value="featured" {{ old('status', $product->status) == 'featured' ? 'selected' : '' }}>Featured</option>
                                            ...
                                            @foreach($currentImages as $index => $image)
                                                <div class="relative group">
                                                    ...
                                                    <div class="absolute inset-0 bg-black bg-opacity-50 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                        <span class="text-white text-sm">Existing Image</span>
                                                    </div>
                                                </div>
                                            @endforeach
                                        </div>
                                        <p class="text-sm text-gray-500">Upload new images below to replace existing ones</p>
                                    </div>
                                @endif

                                <!-- Upload Photos -->
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 mb-3">Upload New Photos (Optional) <span class="text-gray-500 text-xs">(Max 5MB per image)</span></label>
                                    <div id="dragDropArea"
                                ...
                                <div class="pt-4 border-t border-gray-200">
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <textarea name="description"
                                            class="w-full px-4 py-3 border border-green-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-300 focus:border-green-300 transition-colors"
                                            rows="3" placeholder="Enter a brief description of your product">{{ old('description', $product->description) }}</textarea>
                                        @error('description')
                                            <span class="error-message text-red-500 text-sm mt-1 block">{{ $message }}</span>
                                        @enderror
                                    </div>
                                    ...
                                    <!-- Specifications -->
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 mb-2">Specifications (weight, grade, packaging type, harvest date)</label>
                                        <textarea name="specifications"
                                            class="w-full px-4 py-3 border border-green-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-300 focus:border-green-300 transition-colors"
                                            rows="4" placeholder="Enter specifications">{{ old('specifications', $product->specifications) }}</textarea>
