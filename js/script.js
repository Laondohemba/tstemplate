// Sidebar toggle functionality - Wrapped in DOMContentLoaded
document.addEventListener('DOMContentLoaded', function() {
    ...
    window.addEventListener('resize', function () {
        if (window.innerWidth >= 1024) {
            ...
            // Don't auto-close on resize to mobile if user hasn't opened it
            if (!sidebar.classList.contains('-translate-x-full')) {
                // Sidebar is open, keep it open
            }
        }
    });
...
function initForm() {
    ...
    // Check if form elements exist (only run on add product page)
    if (!stepText || !stepTitle || !progressBar) {
        return;
    }
...
// My order - Calendar functionality
document.addEventListener('DOMContentLoaded', function() {
    const calendar = document.getElementById('calendar');
    
    // Only initialize calendar if it exists on the page
    if (!calendar) return;
    ...
    function renderCalendar() {
        ...
        cal.innerHTML = days.slice(0, 35).map(d => `
                <div class="calendar-day text-center py-1 sm:py-2.5 rounded-md text-xs sm:text-sm font-medium cursor-pointer ${d.selected ? 'selected' :
                d.disabled ? 'disabled' :
                    ''
            }">
                    ${d.day}
...
// close banner functionality for info banner
